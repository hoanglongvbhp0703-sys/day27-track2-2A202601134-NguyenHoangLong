...
def test_multiwindow_sustained_fast_burn_pages():
    res = multiwindow_burn(short_window_burn=16.0, long_window_burn=8.0)
    assert res["page"] is True
    assert res["severity"] == "critical"


def test_multiwindow_transient_spike_warns_no_page():
    res = multiwindow_burn(short_window_burn=20.0, long_window_burn=2.0)
